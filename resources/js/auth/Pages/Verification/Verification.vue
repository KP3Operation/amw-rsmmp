...
export default {
    ...
    validations() {
        return {
            otpForm: {
                code: {
                    required: helpers.withMessage("Kode OTP tidak boleh kosong", required)
                }
            }
        }
    },
    methods: {
        ...
        async otpVerification() {
            ...
            axios.post('/api/v1/verification', {code: this.otpForm.code})
                .then((response) => {
                    if (this.isRegistration) {
                        ...
                        if (this.userData.userRole === 'patient') {
                            ...
                            this.updateUserPatientData({
                                ...
                                gender: (response.data.userPatient.gender === 'F') ? 'Perempuan' : 'Laki-Laki',
        ...
        resendOtpCode() {
            ...
            if (this.userData.phoneNumber === '') {
                this.toggleErrorAlert('Terjadi kesalahan. Mohon untuk me-refresh halaman browser', true);
                this.updateLoadingState(false);
                return;
            }
            ...
            if (!this.isCountDownRunning) {
                axios.post('/api/v1/login', {
                    phoneNumber: this.userData.phoneNumber.replace(
                        import.meta.env.VITE_APP_CALLING_CODE,
                        ""
                    )
                }).then((response) => {
                    this.updateOtpData({
                    ...
                    this.updateUserData({phoneNumber: response.data.phoneNumber});

                    this.lastCodeUpdateDate = new Date(this.otpData.otpUpdatedAt);
                    this.futureDateTime = this.lastCodeUpdateDate.getTime() +
                        this.otpData.otpTimeout;
                    this.paddedResendOtpTimeout = (this.otpData.otpTimeout / 1000).toString()
    ...
    mounted() {
        ...
        this.lastCodeUpdateDate = new Date(this.otpData.otpUpdatedAt);
        this.futureDateTime = this.lastCodeUpdateDate.getTime() + this.otpData.otpTimeout;
        this.paddedResendOtpTimeout = (this.otpData.otpTimeout / 1000).toString()
        this.countDown();
    }
}
...
<template>
    <div>
        ...
        <form id="verification-form" @submit.prevent="otpVerification" class="mt-5">
            ...
            <div class="d-flex flex-column mt-3">
                <SubmitButton :text="$t('verification.verify')"
        ...
        <p class="mt-4 text-center" v-show="!isLoading">
            ...
            <a href="javascript:void(0);" @click="resendOtpCode"
