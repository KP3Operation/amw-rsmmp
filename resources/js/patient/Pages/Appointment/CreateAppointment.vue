...
export default {
  ...
  setup() {
    ...
    const rules = {
      patient_name: { required: helpers.withMessage("Nama pasien tidak boleh kosong", required) },
      patient_id: { },
      gender: {
      ...
      appointment_date: {
        required: helpers.withMessage("Tanggal konsultasi tidak boleh kosong", required),
      },
      service_unit_id: {
        required: helpers.withMessage("Unit tidak boleh kosong", required),
      },
      paramedic_id: {
        required: helpers.withMessage("Dokter tidak boleh kosong", required),
      },
    }
    ...
    const storeAppointment = async () => {
      ...
      if ((form.patient_name === null || form.patient_name === form.patient_id) && tempPatientName.value !== "") {
        form.patient_name = tempPatientName.value;
      }

      apiRequest.post('/api/v1/patient/appointments/store', form).then((response) => {
    ...
    watch(() => form.patient_name, (newValue, oldValue) => {
      if (newValue === '') {
        form.patient_id = '';
        form.birth_date = '';
        form.gender = '';
        ...
        const foundFamily = families.value.find(family => family.patient_id === newValue);
        if (foundFamily) {
    ...
    watch(() => form.appointment_date, (newValue, oldValue) => {
    ...
    watch(() => form.service_unit_id, (newValue, oldValue) => {
      if (newValue && !isFromDoctorSchedulePage.value) {
        const serviceUnit = serviceUnits.value.find(unit => unit.serviceUnitID === newValue);
        if (serviceUnit) {
    ...
    <div class="px-4 pt-7 pb-4">
        ...
        <form @submit.prevent="storeAppointment" class="d-flex flex-column rows-gap-16 mt-3">
            ...
            <div :class="{ error: v$.patient_name.$errors.length }">
                ...
                <select name="nama" id="nama" class="form-select mt-2"
                        v-model="form.patient_name"
                        @input="v$.patient_name.$touch()">
                    <option value="">Pilih Member</option>
                    <option :value="userPatientData.patientId">{{ userData.userFullName }}</option>
            ...
            <div :class="{ error: v$.patient_id.$errors.length }">
                ...
                <input type="text" class="form-control mt-2"
                       ...
                       readonly>
              <div class="error mt-2 fs-6 fw-bold text-red-200" v-for="error of v$.patient_id.$errors"
                   :key="error.$uid">
                {{ error.$message }}
              </div>
            </div>
            <div :class="{ error: v$.birth_date.$errors.length }">
                ...
                <input type="date"
                       v-model="form.birth_date" name="dob" id="dob"
            ...
            <div v-if="!isFromDoctorSchedulePage">
                ...
                <div class="mt-3" :class="{ error: v$.appointment_date.$errors.length }">
                    ...
                    <input v-model="form.appointment_date"
                           type="date"
                           name="date"
                           id="date"
                           class="form-control mt-2"
            ...
            <div v-if="isFromDoctorSchedulePage">
                ...
                <div class="mt-3" :class="{ error: v$.appointment_date.$errors.length }">
                    <label for="date">Tanggal Konsultasi <span class="text-red-500 fw-semibold">*</span></label>
                    <input v-model="form.appointment_date"
                           type="date" name="date" id="date" class="form-control mt-2"
                           @input="v$.appointment_date.$touch()">
                  <div class="error mt-2 fs-6 fw-bold text-red-200" v-for="error of v$.appointment_date.$errors"
                       :key="error.$uid">
                    {{ error.$message }}
