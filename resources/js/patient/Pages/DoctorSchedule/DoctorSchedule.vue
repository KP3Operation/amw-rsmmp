...
const { schedules, selectedDate, selectedServiceUnitId } =
...
watch(selectedDate, (newValue, oldValue) => {
    if (newValue !== "") {
        ...
        doctorScheduleStore.updateSelectedDate(
            convertDateToFormField(new Date())
        );
        fetchDoctorSchedules();
    }
});
...
<template>
    ...
    <div class="px-4 pt-8 mt-4">
        ...
        <section class="d-flex flex-column rows-gap-16 mt-4" v-if="!isLoading">
            <div
                class="bg-blue-100 rounded px-4 py-3"
                v-for="schedule in schedules"
            >
                <p class="fw-semibold">{{ schedule.paramedicName }}</p>
                <p class="mt-2 fs-5 text-gray-700">
                    {{ schedule.serviceUnitName }}
                </p>
                <router-link
                    @click="selectParamedicId(schedule.paramedicID)"
                    :to="{
