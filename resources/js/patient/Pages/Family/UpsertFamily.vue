...
export default {
    ...
    setup() {
        ...
        const updateFamily = () => {
            layoutStore.isLoading = true;
            apiRequest
                .put(`/api/v1/patient/family/${route.params.id}`, form)
                .then((response) => {
                    const data = response.data.data;
                    layoutStore.toggleInfoAlert(
                        `Berhasil Mengubah Data Member ${data.family.name}`
                    );
                    router.push({ path: "/family" });
                })
                .catch((error) => {
        ...
        const fetchFamily = () => {
            layoutStore.isLoading = true;
            apiRequest
                .get(`/api/v1/patient/family/${route.params.id}`)
                .then((response) => {
                    const data = response.data;
                    form.id = data.family.id;
                    form.name = data.family.name;
                    form.ssn = data.family.ssn;
                    form.phone_number = data.family.phone_number.replace(
                        callingCode,
                        ""
                    );
                    form.gender =
                        data.family.gender === "F" ||
                        data.family.gender === "Perempuan"
                            ? "Perempuan"
                            : "Laki-Laki";
                    form.birth_date = convertDateToFormField(
                        data.family.birth_date
                    );
        ...
        onMounted(() => {
            ...
            modalState.familyDataConfirmation = new bootstrap.Modal(
                "#modal-konfirmasi"
            );
...
<template>
    <Header
        :title="
            isEditMode ? $t('family.edit.title') : $t('family.create.title')
        "
        :with-back-url="true"
        pageName="FamilyPage"
        :with-action-btn="false"
    >
    </Header>
        ...
        <div class="modal-dialog modal-dialog-centered">
            <div class="modal-content">
                <div class="modal-header d-flex justify-content-between">
                    <div class="d-flex align-items-center col-gap-8">
                        ...
                        <h5 class="modal-title">
                            {{ $t("family.create.confirmation_modal.title") }}
                        </h5>
                    </div>
                    <button
                        type="button"
                        class="btn-close"
                        @click="modalState.familyDataConfirmation.hide()"
                        aria-label="Close"
                    >
                ...
                <div class="modal-footer flex-nowrap">
                    ...
                    >
                        {{ $t("family.create.confirmation_modal.no") }}
                    </button>
                    <button
                        @click="storeFamilyConfirmTrue"
                        type="button"
                        class="w-50 btn-next btn btn-blue"
                    >
                        {{ $t("family.create.confirmation_modal.yes") }}
                    </button>
                </div>
            </div>
        </div>
