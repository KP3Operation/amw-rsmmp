...
<template>
    <div>
        <Header :title="isEditMode ? $t('family.edit.title') : $t('family.create.title')" :with-back-url="true" pageName="FamilyPage" :with-action-btn="false">
        </Header>
        ...
        <div class="px-4 pt-7 pb-4">
            ...
            <form @submit.prevent="isEditMode ? updateFamily() : storeFamily()" @keydown="form.onKeydown($event)"
                ...
                <div>
                    <label for="nohp">{{ $t('family.phone_number') }} <span
                            class="text-red-500 fw-semibold">*</span></label>
                ...
                <SubmitButton className="btn-blue-500-rounded" :text="isEditMode ? $t('family.edit.save') : $t('family.save')" />
                <router-link v-show="!layoutStore.isLoading" to="/family" class="text-center text-blue-500 text-decoration-none fw-bold">{{
